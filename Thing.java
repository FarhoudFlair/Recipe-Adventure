...
public class Thing{
 ...
 public void interact(Player p){
   // Adds Thing to inventory of Player
   p.getThings().add(this);
    List<Thing> inventory = p.getWorld().getRoom(p).getThings();
    for (int i=0; i<inventory.size(); i++) {
      if (inventory.get(i).equals(this)) {
        inventory.remove(i);
      }
    }
    System.out.println(this.getName() + " has been added to your inventory.");
 }
